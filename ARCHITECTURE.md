...
- **Proxy:** Proteção de rotas `/dashboard`, `/app` e `/admin` via `src/proxy.ts`.
...
### 9. Next.js 16 Naming Conventions
- **Proxy:** Não utilize `middleware.ts`. Use obrigatoriamente `src/proxy.ts` para interceptação de rotas e gestão de sessões.
- **Cookie Manipulation:** Ao usar `request.cookies.set`, sempre passe um objeto único `{ name, value, ...options }` para conformidade com a tipagem do Next.js 16.

### 10. Supabase Key Standards (2025 Update)
- **Publishable Key:** Utilize `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` (formato `sb_publishable_...`). Substitui o antigo `anon_key`.
- **Secret Key:** Utilize `SUPABASE_SECRET_KEY` (formato `sb_secret_...`). Substitui o antigo `service_role`.
- **Validation:** Sempre verifique a existência das variáveis antes de instanciar o `createServerClient` para evitar crashes no Proxy/Middleware.
